...
class InferenceAgent:
    ...
    def run_inference(self, res_path, ref_path, aud_path, pose_path=None, gaze_path=None, **kwargs):
        ...
        if getattr(self.opt, "static_pose_zero", False):
            a = data["a"]
            seq = int(a.shape[-1])
            t_pose = max(1, int(math.ceil(seq * float(self.opt.fps) / float(self.opt.sampling_rate))))
            data["pose"] = torch.zeros(t_pose, 3, device=self.opt.rank, dtype=torch.float32)
            scv = getattr(self.opt, "static_cam_values", None)
            if scv is not None:
                cam_vec = torch.tensor(scv, device=self.opt.rank, dtype=torch.float32).view(3)
                data["cam"] = cam_vec.unsqueeze(0).expand(t_pose, -1).contiguous()
            elif getattr(self.opt, "static_ref_cam", False):
                from generator.ref_smirk_cam import first_frame_cam_from_ref_tensor

                ck = (getattr(self.opt, "smirk_encoder_ckpt", None) or "").strip() or None
                cam_vec = first_frame_cam_from_ref_tensor(
                    data["s"],
                    smirk_root=str(getattr(self.opt, "smirk_root", "/workspace/SMIRK")),
                    ckpt_path=ck,
                    device=torch.device(self.opt.rank),
                    image_size=int(getattr(self.opt, "smirk_image_size", 224)),
                    mediapipe_crop=not bool(getattr(self.opt, "smirk_no_mediapipe_crop", False)),
                )
                data["cam"] = cam_vec.unsqueeze(0).expand(t_pose, -1).contiguous()
            else:
                data["cam"] = None
        elif pose_path and os.path.exists(pose_path):
...
class InferenceOptions(BaseOptions):
    def initialize(self, parser):
        ...
        parser.add_argument(
            "--static_pose_zero",
            action="store_true",
            help="Ignore pose_path: use pose (0,0,0) for every frame (length from audio). "
            "Cam: --static_cam_values, else --static_ref_cam, else omit.",
        )
        parser.add_argument(
            "--static_cam_values",
            nargs=3,
            ...
            help="With --static_pose_zero: fixed cam (3 floats) every frame; overrides --static_ref_cam.",
        )
        parser.add_argument(
            "--static_ref_cam",
            action="store_true",
            help="With --static_pose_zero: SMIRK on preprocessed ref → cam (3,) repeated every frame.",
        )
        parser.add_argument(
            "--smirk_root",
            type=str,
            default=os.environ.get("SMIRK_ROOT", "/workspace/SMIRK"),
            help="SMIRK repo root on sys.path.",
        )
        parser.add_argument(
            "--smirk_encoder_ckpt",
            type=str,
            default=os.environ.get("SMIRK_ENCODER_CKPT", ""),
            help="Encoder weights; default <smirk_root>/pretrained_models/SMIRK_em1.pt when empty.",
        )
        parser.add_argument(
            "--smirk_image_size",
            type=int,
            default=int(os.environ.get("SMIRK_IMAGE_SIZE", "224")),
            help="SMIRK encoder input size.",
        )
        parser.add_argument(
            "--smirk_no_mediapipe_crop",
            action="store_true",
            help="Skip MediaPipe face crop before SMIRK.",
        )
...
def process_item(agent, ref, aud, name, opt):
    ...
    res_vp = getattr(opt, "res_video_path", None)
    if res_vp:
        out_path = res_vp
